        ...
        if file:
            ...
            if result is not None:
                ...
                if upload_to_postgres(result, table_name):
                    # Store the table name in the application context
                    app.config['LAST_UPLOADED_TABLE'] = table_name
...
def get_data():
    try:
        ...
        with engine.connect() as connection:
            table_name = app.config.get('LAST_UPLOADED_TABLE', 'uob_transactions')
            query = text(f'SELECT * FROM {table_name} ORDER BY "Date" DESC LIMIT 1000')
